...
来源是整个系统的第一优先级。用户可以通过文件上传、网页链接、文本粘贴、快速搜索和深度研究导入素材；文件上传支持 PDF、Word、图片、音频、视频、CSV 等格式，也支持一次选择多个文件，适合把论文、课件、网页资料、截图、课程录音、演示视频和结构化数据统一放进同一个知识库。

导入后，用户可以在左侧栏选择参与当前对话和产出的材料，中间对话区会围绕这些来源回答问题。回答旁边提供沉淀入口，方便把单条回答、一轮问答或多条消息推送到右侧知识资产。

![添加来源](docs/assets/showcase/来源展示.png)
...
PDF 来源除了正文解析外，还支持提取页面图和内嵌图片，并在左侧来源区提供图片索引重建、PDF 图片查看等入口。系统会把这些图片内容纳入视觉检索和问答上下文，方便用户围绕图表、流程图、页面截图等视觉证据继续提问。

这类 PDF 图片证据会统一沉淀到笔记本目录下，并参与后续对话、文档沉淀和报告/PPT 等产出。

![来源工作区](docs/assets/thinkflow/workspace-sources.png)
