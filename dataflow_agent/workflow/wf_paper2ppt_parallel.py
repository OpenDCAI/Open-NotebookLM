...
def create_paper2ppt_parallel_graph() -> GenericGraphBuilder:  # noqa: N802
    ...
    async def generate_pages(state: Paper2FigureState) -> Paper2FigureState:
        """
        批量生成/编辑页面图（并行版本）：
        - pagecontent 由上游 kb_page_content 的 LLM 大纲生成，无人工确认。
        - 按 asset 规则决定 text2img / img2img；或直接为图片路径列表时逐页编辑。
        并发调用 asyncio.gather 处理所有页面。
        """
        import asyncio
        page_items = state.pagecontent or []
        log.info("[paper2ppt] 使用 pagecontent 生图，共 %s 页（大纲由 LLM 生成，无人工确认）", len(page_items))
        ...
        async def _process_single_page(idx: int, item: Any) -> Dict[str, Any]:
            ...
            log.info(
                "[paper2ppt] page=%s/%s title=%s use_edit=%s save=%s",
                idx + 1, len(state.pagecontent or []), title, use_edit, save_path,
            )
        ...
        log.info("[paper2ppt] 并发生图开始，共 %s 页", len(tasks))
        ...
        log.info("[paper2ppt] 并发生图完成，耗时 %.2fs", cost_time)
