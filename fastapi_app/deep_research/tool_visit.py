...
def get_jina_api_keys():
    """Dynamically get JINA_API_KEYS from environment"""
    return os.getenv("JINA_API_KEYS", "")
...
class Visit(BaseTool):
    ...
    def jina_readpage(self, url: str) -> str:
        ...
        jina_keys = get_jina_api_keys()
        if not jina_keys:
            print("[visit] JINA_API_KEYS not configured, skipping Jina service")
            return "[visit] Failed to read page."

        for attempt in range(max_retries):
            headers = {
                "Authorization": f"Bearer {jina_keys}",
            }
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"[visit] Jina API authentication failed (401). Please check your JINA_API_KEYS.")
                    return "[visit] Failed to read page."
                else:
                    print(f"[visit] Jina API returned status {response.status_code}: {response.text}")
                    if attempt == max_retries - 1:
                        return "[visit] Failed to read page."
            except Exception as e:
                print(f"[visit] Jina API error on attempt {attempt + 1}/{max_retries}: {str(e)}")
                time.sleep(0.5)
                if attempt == max_retries - 1:
                    return "[visit] Failed to read page."
